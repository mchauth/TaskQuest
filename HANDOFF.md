...
├── scripts/
│   ├── gen_weapons_v2.py               ← weapon sprite generator (sword/staff/bow)
│   ├── add_projectiles.py              ← post-processor: animated arrow + orb on slash frames
│   ├── sprite_finish.py                ← finishing pass (shading, visors) — EVERY sheet runs through this
│   ├── sprite_shade.py                 ← low-level shader
│   ├── sprite_qa.py                    ← QA checker
│   ├── sprite_pipeline.py              ← shade → QA → auto-fix loop
│   ├── rebuild_class_hats.py           ← hat authoring
│   ├── propagate_rare_helmets.py       ← frame propagation for rare helms
│   └── redesign_tiers_v3.py           ← tier armor authoring
├── sprites/
...
## ⚠️ BOW DESIGN LOCK-IN (READ THIS FIRST)

**The approved bow design comes from commit `bbcc37d`** ("Bows: restore original design flipped 180° on both axes"). This is a classic ')' curve — not an S-curve. It has been repeatedly lost to regeneration accidents.

**Rule: NEVER call `make_bow_frame0()` in gen_weapons_v2.py to regenerate bows.**  
The bow generation loop MUST use `extract_f0(existing_file)` to read the existing approved design, then build the slash animation around it. The design lives in the PNG files, not in code.

```python
# CORRECT — preserves approved design:
for tier in ['t1','t2','t3','t4','t5','t6']:
    for g in ['m','f']:
        fname = f'{OUT_DIR}bow_ranger_{tier}_{g}.png'
        if not os.path.exists(fname):
            print(f"  SKIP (not found): {fname}"); continue
        f0 = extract_f0(fname)   # ← reads frame 0 from existing approved file
        build_sheet(f0, SRC_PATH, fname, weapon_type='bow', ...)

# WRONG — destroys the approved design:
# f0 = make_bow_frame0(...)
```

**To restore the approved bow files from git:**
```bash
git checkout bbcc37d -- sprites/preview_assets/char/bow_ranger_*.png
```

**make_bow_frame0() still exists in gen_weapons_v2.py** — do not delete it (it's useful for reference), but do NOT call it during bow generation.

---

## Weapon Sprite System (scripts/gen_weapons_v2.py)

All weapon sprites are 800×448px sprite sheets (same format as character layers). Generated by `scripts/gen_weapons_v2.py`.

### Sprite Sheet Format
- **Dimensions:** 800×448px, 80×64px per frame
- **Grid:** 10 columns × 7 rows = 70 frame slots
- **Frame index:** `fi = row*10 + col`
- **Row 5 = Slash animation** — 6 frames: fr50–fr55, fps=10

### Sword Generation

Sword slash frames (fr51–fr53) are rotated using PCA-extracted angles from sword.png:

```python
SLASH_ANGLES = {51: -35.0, 52: 4.8, 53: 15.5}  # degrees, from PCA on sword.png

# +180° is REQUIRED to correct blade orientation (grip in hand, blade swinging):
angle_delta = SLASH_ANGLES.get(fi, 0.0) + 180.0
```

**Critical:** without the +180°, the sword blade appears upside down during the swing (grip swings, blade stays in hand). This was confirmed by the user — always keep the +180°.

### Bow/Staff Generation — Centroid Alignment

Bow and staff are positioned via centroid-based translation so they track the character's hand across all animation frames.

**Key centroid values (measured from source sprites):**
| Weapon | Frame 0 centroid (cx, cy) |
|--------|--------------------------|
| Sword  | (35.4, 51.5) |
| Bow    | (39.2, 42.4) — 9.1px above sword |
| Staff  | (33.9, 51.7) — very close to sword |

**Alignment formula:**
```python
actual_dx = round(target_cx - cx0_f0)   # cx0_f0 = weapon's own fr0 centroid x
actual_dy = round(target_cy - cy0_f0)   # target = sword hand centroid for that frame
```

### Staff Energy Orb Position

The energy orb for frames 51–53 is placed relative to the staff centroid (NOT topmost pixel):
```python
orb_x = round(target_cx) - 6
orb_y = round(target_cy) - 6
```
Using top-of-staff pixel was wrong (sent orb off-screen at y≈-21). Centroid-relative placement is stable.

### Workflow

```bash
# 1. Generate sword and staff sheets:
cd ~/Projects/TaskQuest
python scripts/gen_weapons_v2.py

# 2. Add animated projectiles (arrow + orb) to slash frames:
python scripts/add_projectiles.py

# NEVER run gen_weapons_v2.py for bow without verifying extract_f0 is used (not make_bow_frame0)
```

---

## Projectile System (scripts/add_projectiles.py)

Post-processing script that adds animated projectiles to weapon slash frames WITHOUT touching frame 0 (preserving the bow design).

### Arrow (Bow — ranger)

- Animated across frames 51 → 52 → 53
- Moves RIGHT across the screen (characters are designed facing left, flipped right for display)
- `ax_start = round(cx) + 4 + i * 18` where i=0,1,2 for frames 51,52,53
- Arrow is ~10px long, rightward direction
- Script erases existing arrow pixels by color match before redrawing (prevents doubles)

### Energy Orb (Staff — mage)

- Animated across frames 51 → 52 → 53
- Arcs rightward from wand tip toward enemy
- `orb_x = tip_x + i * 20`; `orb_size = 2 + i` (grows as it travels)
- Per-tier color table: `STAFF_TRAILS` dict with `(center_color, edge_color)` per tier

### Why separate from gen_weapons_v2.py?

Keeping projectiles as a post-processor means bow frame 0 is NEVER touched during projectile addition — the generator only builds the swing animation, and the post-processor adds motion effect sprites to fr51–53 only.

---

## Combat Animation System

### Class-Based Attack Sequences

Three separate animation systems in index.html all use class-aware frame sequences:

| Class   | Slash frame sequence | Notes |
|---------|---------------------|-------|
| Warrior | `[0,1,2,3,4,5]` | Full 6-frame slash |
| Mage    | `[0,5,4,4,5,0]` | Arm raise, hold at peak (fr54/55), lower |
| Ranger  | `[0,5,4,4,5,0]` | Same as mage |

Mage/ranger use only the LAST two slash frames (fr54=arm pointing forward, fr55=arm extended) — NOT the first two. The sequence starts from idle frame 0, raises to the attack pose, holds, then lowers.

**Three locations in index.html where this appears:**

1. **Home screen tap attack** (`hsDoSlash`, ~line 1918):
   ```js
   var frameSeq = (cls === 'warrior') ? [0,1,2,3,4,5] : [0,5,4,4,5,0];
   ```

2. **Combat victory overlay** (`openCombatOverlay`, ~line 3232):
   ```js
   var _coFrameSeq = (_coCls === 'warrior') ? [0,1,2,3,4,5] : [0,5,4,4,5,0];
   ```

3. **Quest scene combat loop** (`qsLoop`, ~line 4142):
   ```js
   if (_qsCls2 !== 'warrior') {
     displayFrame = [0,5,4,4,5,0][qsCharFrame % 6];
   }
   ```

### Quest Scene Combat Loop (qsLoop)

The quest scene auto-attacks at the same fps as the home screen tap animation, with idle animation between attacks:

```js
var QS_SLASH_PAUSE = 1500;    // ms between slash cycles
var qsSlashCooldown = 0;      // timestamp when next attack begins
var qsIdleLast = 0;           // last idle frame tick
var qsIdleFrame = 0;          // current idle frame index

// In qsLoop:
if (qsAnim === 'combat') {
  if (ts < qsSlashCooldown) {
    // Play idle (bobbing) animation during cooldown
    var idleAnim = ANIM.idle;
    if (ts - qsIdleLast > 1000 / idleAnim.fps) {
      qsIdleLast = ts;
      qsIdleFrame = (qsIdleFrame + 1) % idleAnim.frames;
      qsSetCharFrame(idleAnim, qsIdleFrame);
    }
  } else {
    // Play slash animation
    if (ts - qsCharLast > 1000 / anim.fps) {
      qsCharLast = ts;
      qsCharFrame = (qsCharFrame + 1) % anim.frames;
      var displayFrame = qsCharFrame;
      if (_qsCls2 !== 'warrior') {
        displayFrame = [0,5,4,4,5,0][qsCharFrame % 6];
      }
      qsSetCharFrame(anim, displayFrame);
      if (qsCharFrame === anim.frames - 1) {
        qsSlashCooldown = ts + QS_SLASH_PAUSE;  // start cooldown after full cycle
        qsCharFrame = 0;
      }
    }
  }
...
The character bobs in idle between attacks instead of freezing — matches the home screen behavior.
...
**Class-based combat animation:** Each class plays a different frame sequence during attacks. See "Combat Animation System" section above for full details. Mage and Ranger use `[0,5,4,4,5,0]`; Warrior uses `[0,1,2,3,4,5]`. This applies in all three animation systems (home screen tap, quest scene loop, combat overlay).
...
### Weapon / Animation (recent work — pick up here)

1. **Staff position still slightly off:** The staff floats a couple pixels left of the hand in some frames. Centroid alignment is mathematically correct (staff centroid ≈ sword centroid) but a visual offset remains. May need a manual DX tweak in gen_weapons_v2.py.
2. **Bow size increase (careful):** User wants bows slightly bigger, but every attempt to scale has destroyed the approved ')' design. Need a scale approach that enlarges without touching frame 0 shape — consider PIL resize on frame 0 only, then re-extract. Do NOT call make_bow_frame0().
3. **Verify projectile visibility on live site:** Animated arrow (fr51→52→53) and orb (fr51→52→53) were added in commit `d028622`. Confirm these are visually clear in the quest scene.

### Armor / Helmets

4. **Armor tiers 3–6 (`armor_chest_3.png` through `armor_chest_6.png`):** Design and generate the remaining chest armor progression (chainmail → plate, etc.) using the confirmed shirt-mask method (see Armor Sprite Pipeline above).
5. **Cowl helmets (mage + ranger legendary):** Generate `helmet_mage_legendary_cowl.png` and `helmet_ranger_legendary_cowl.png` (+ `_f` variants) via `scripts/redesign_cowl_helmets.py`. Hood cap + hanging cape only — chest area transparent. Register with `hatType:'hood'` in LOOT_TABLE. See "Hood / Cowl Helmets" section in the Helmet Pipeline above for hair-rendering behavior.
6. **Divine Seraph Plate with Wings — female version:** `shirt_warrior_legendary1_f.png` via `scripts/legendary_armor_t1.py`. Male is already staged in CONTEXT.md awaiting approval; female version is still needed.
7. **Class-filtered loot content:** Currently only warrior items have rarity/class fields. Populate Mage and Ranger loot with correct `classes` values and matching sprites.

### Hair

8. **Hair re-generation:** Re-do Long (style 2) and Ponytail (style 4) with full-head inpaint mask (not just extension) so short hair doesn't bleed through. Remove or redo styles 3/5/6.
9. **Female hairstyles:** All female styles (hair_f1–5) are currently just 5 color variants of one short style. Need the same style expansion as male.

### Other

10. **More PixelLab assets:** Trees, props, and other scene objects via Chrome API pipeline.
