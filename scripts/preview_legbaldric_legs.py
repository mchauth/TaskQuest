#!/usr/bin/env python3
"""Preview the diagonal sword-belt legs (LEGS 7th axis) for daily approval. For
each class and gender: full avatar (skin + base shirt + these legs + boots + hair)
across idle/walk/run/cheer/slash, plus one isolated legs-over-skin so the net-new
DIAGONAL sword-belt geometry is clear. Writes to $OUTP (default
_PREVIEW_legbaldric_legs.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_legbaldric_legs_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_legbaldric_legs.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "pants_warrior_legendary7", "Warlord's Sword-Belt"),
    ("mage", "pants_mage_legendary7", "Astral Sash"),
    ("ranger", "pants_ranger_legendary7", "Warden's Baldric-Belt"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, LEGENDARY legs — net-new DIAGONAL sword-belt geometry, 7th legs axis)",
