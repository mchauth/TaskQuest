#!/usr/bin/env python3
"""Preview the horned legendary helmet (Wyrmhorn Warhelm) — full-avatar
composites (skin+hair+body+horned helm) across idle/walk/run/cheer frames, male
and female, to show the horn silhouette tracking the head across the animation.
Writes to the current session outputs for daily approval."""
import os
import numpy as np
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_horned_legendary_preview"
FW, FH, COLS = 80, 64, 10
OUT = "/sessions/elegant-youthful-pasteur/mnt/outputs/PREVIEW_horned_legendary.png"
...
def avatar(gender, fi):
    ...
    shirt = Image.open(f"{CH}/shirt_rare1{suf}.png").convert("RGBA")
    pants = Image.open(f"{CH}/pants_rare1{suf}.png").convert("RGBA")
    helm = Image.open(f"{PREV}/helmet_warrior_legendary1{suf}.png").convert("RGBA")
    ...
    base.alpha_composite(frame(helm, fi))    # horned helm over hair
...
def main():
    ...
    d.text((pad, 16),
           "CONCEPT — Wyrmhorn Warhelm (warrior hyper-rare)  ·  net-new horn geometry  ·  staged, NOT pushed",
