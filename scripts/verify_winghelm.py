#!/usr/bin/env python3
"""Rigorous per-sheet verification for the winged-helm legendaries.
For each of the 6 sheets, vs its source helmet sheet:
  ...
  3. accent-caused multi-component frames = frames whose OUTPUT has >1 connected
     component beyond what the SOURCE already has (=0 -> every wing fused)
  4. accent strays = output opaque pixels neither in-source nor 4-connected
     (via the accent mass) to the body (=0)
"""
import numpy as np
...
PAIRS = [
    ("helmet_rare1", "helmet_warrior_legendary3"),
    ("helmet_mage4", "helmet_mage_legendary3"),
    ("helmet_ranger4", "helmet_ranger_legendary3"),
]
...
def check(src_stem, dst_stem, suf):
    ...
    for fi, sf in frames(src):
        ...
        strays += int((da & ~sa & ~connected).sum())
