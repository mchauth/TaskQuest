#!/usr/bin/env python3
"""Preview the folded-cuff cavalier boots for daily approval. For each class and
gender: full avatar (skin + base pants + base shirt + cuff boots + hair) across
idle/walk/run/cheer/slash, plus one isolated boots-over-skin so the net-new low
wide folded-cuff geometry is clear. Writes to $OUTP (default _PREVIEW_cuff_boots.png)."""
import os
...
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_cuff_boots.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "boots_warrior_legendary_cuff", "Bulwark Warboots"),
    ("mage", "boots_mage_legendary_cuff", "Nightveil Striders"),
    ("ranger", "boots_ranger_legendary_cuff", "Pathwarden Boots"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE boots — net-new low-wide folded-cuff cavalier geometry)",
        ...
        for gender in ("m", "f"):
            ...
            d.text((x + 2, yy + ch * Z), f"{gender} iso (cuff+skin)", font=fsm, fill=(200, 200, 210, 255))
            yy += row_h
        y += class_h + pad
