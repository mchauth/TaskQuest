#!/usr/bin/env python3
"""Preview the aventail (camail) helmets for daily approval. For each class and
gender: full avatar (skin + base pants + base shirt + hair + aventail helmet)
across idle/walk/run/cheer/slash, plus one isolated helmet-over-skin so the
net-new downward hanging-drape geometry is clear. Writes to $OUTP (default
_PREVIEW_aventail_helmet.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_aventail_helmet_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_aventail_helmet.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "helmet_warrior_legendary4", "Ironmaw Camail"),
    ("mage", "helmet_mage_legendary4", "Nightweave Veil"),
    ("ranger", "helmet_ranger_legendary4", "Thornmesh Aventail"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE helmet — net-new downward hanging-aventail geometry)",
