#!/usr/bin/env python3
"""Generate the Mage 4th legendary L25 set — "Malachite Magus".

Follows gen_mage_legendary3.py exactly (luminance-quantile color transfer of the
...
Ramp is a rich malachite-green family (deep forest shadow -> jade -> bright
verdant -> pale mint glint). Distinct from the mage tiers (purple/void family)
AND from all three existing mage legendaries: Astral rare1 (cyan), Ember rare2
(orange), Bloodmoon rare3 (red). Mage has no green anywhere else, so Malachite
reads clearly as its own fourth legendary.

Outputs to _mage_legendary4_preview/ (staged, not pushed).
"""
import os
import numpy as np
...
# Malachite ramp, darkest -> lightest. Deep forest -> jade -> pale mint.
RAMP = np.array([
    (8, 30, 18),     # deep forest shadow
    (12, 50, 28),    # dark green
    (18, 78, 42),    # green
    (30, 110, 62),   # verdant
    (54, 146, 88),   # bright green
    (100, 184, 124), # light jade
    (170, 216, 184), # pale mint
    (232, 248, 236), # near-white mint highlight
], dtype=np.uint8)
...
def main():
    ...
    for slot, base_name in SLOTS.items():
        for suf in ('', '_f'):
            ...
            out_name = f'{slot}_rare_mage4{suf}.png'
