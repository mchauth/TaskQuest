#!/usr/bin/env python3
"""Preview the crest legendary helmets (Legion War-Crest / Astral Spire Circlet
/ Falcon War-Plume) — full-avatar composites (skin+pants+shirt+hair+crest helm)
across idle/walk/run/cheer/slash, male and female, all three classes, so the
vertical crest silhouette can be checked tracking the head across the animation.
Writes _PREVIEW_crest_legendary.png in the repo root and to the session outputs.
"""
import os
import numpy as np
...
PREV = "_crest_legendary_preview"
FW, FH, COLS = 80, 64, 10
OUT_REPO = "_PREVIEW_crest_legendary.png"
OUT_SESS = "/sessions/zealous-eloquent-gauss/mnt/outputs/PREVIEW_crest_legendary.png"
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (50, "slash")]

# class -> (body-set token, helm out name, label)
CLASSES = [
    ("warrior", "rare1", "helmet_warrior_legendary2", "WARRIOR · Legion War-Crest"),
    ("mage", "mage4", "helmet_mage_legendary2", "MAGE · Astral Spire Circlet"),
    ("ranger", "ranger4", "helmet_ranger_legendary2", "RANGER · Falcon War-Plume"),
]
...
def avatar(token, helm_name, gender, fi):
    ...
    base.alpha_composite(frame(helm, fi))     # crest helm over hair
...
def main():
    ...
    d.text((pad, 14),
           "CONCEPT — Crest legendary helmets (all classes, hyper-rare)  ·  "
           "net-new vertical crest geometry  ·  staged, NOT pushed",
