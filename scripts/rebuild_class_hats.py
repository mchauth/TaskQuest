...
def mage_hat(tier, head_top, cx, brim_drop=0):
    P = MAGE_HAT[tier]
    # brim_drop lowers the whole hat by N px so it rests on the head (female mage
    # hats sat ~2px too high; callers pass brim_drop=2 for female sheets).
    head_top = head_top + brim_drop
    fill, over = {}, {}
...
def ranger_hat(tier, head_top, cx, brim_drop=0):
    head_top = head_top + brim_drop
    P = RANGER_HAT[tier]
    fill, over = {}, {}
...
def build_sheet(builder, tier, frames, head_dome_fn, brim_drop=0):
    ...
    for fi in frames:
        ...
        c, r = fi % COLS, fi // COLS
        gx, gy = c * W, r * H
        for (x, y), rgb in builder(tier, head_top, hcx, brim_drop).items():
            if 0 <= x < W and 0 <= y < H:
...
def main():
    for cls, builder in (('mage', mage_hat), ('ranger', ranger_hat)):
        ...
        if os.path.exists(t1f_path):
            ...
            fdrop = 2 if cls == 'mage' else 0   # female mage hats sit 2px lower
            sheet_f = build_sheet(builder, 1, frames_f, head_dome_f, brim_drop=fdrop)
