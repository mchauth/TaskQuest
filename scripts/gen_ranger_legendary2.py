#!/usr/bin/env python3
"""Generate the Ranger 2nd legendary L25 set — "Frosthunter".

Follows gen_ranger_legendary.py exactly (luminance-quantile color transfer of
...
Ramp is a glacial family (deep steel shadow -> icy blue -> white glint), clearly
distinct from the ranger tiers (dark green family) AND from the ranger rare1
"Verdant Monarch" set (emerald->gold), so it reads as its own second legendary.
The steel-blues are deliberately desaturated (grey-blue), distinct from the
mage "Astral" set's saturated cyan.

Outputs to _ranger_legendary2_preview/ (staged, not pushed).
"""
import os
import numpy as np
...
OUT = '_ranger_legendary2_preview'
FW, FH = 80, 64
...
# Frosthunter ramp, darkest -> lightest. Desaturated steel-blue -> white.
RAMP = np.array([
    (18, 28, 40),    # deep steel shadow
    (30, 50, 72),    # dark slate blue
    (52, 84, 112),   # slate
    (84, 124, 156),  # steel blue
    (128, 168, 196), # pale steel
    (176, 206, 224), # ice
    (216, 234, 244), # near-white ice
    (245, 250, 255), # white glint highlight
], dtype=np.uint8)
...
def main():
    ...
    for slot, base_name in SLOTS.items():
        for suf in ('', '_f'):
            ...
            out_name = f'{slot}_rare_ranger2{suf}.png'
