...
def classify(frame: np.ndarray, protect: bool = True):
    """Return (armor_mask, outline_mask) boolean arrays for one RGBA frame.

    protect=True skips pixels matching the SKIN colour or the HAIR palette.
    That guard exists for sheets which may contain the character's own skin or
    hair. It is WRONG for a standalone armor sheet, which contains garment
    pixels only: a gold/tan/brown helmet has colours sitting squarely inside
    HAIR_PALETTE, so 30-57% of the garment gets misclassified as hair and left
    unshaded while its neighbours are shaded — the sheet comes out patchy, with
    "colored pixels where they don't belong" (Matt 8/1). Measured on frame 0:
    legendary18 52/91 px excluded, legendary36 40/91, legendary25 30/91, vs
    grey legendary9 5/91 and legendary14 0/91 — which is exactly the
    mangled-vs-clean split he reported. sprite_finish passes protect=False.
    """
    ...
    if protect:
        ...
        h, w = rgb.shape[:2]
        diffs = np.abs(rgb.reshape(-1, 1, 3)
                       - HAIR_PALETTE[np.newaxis, :, :]).max(axis=-1)
        ...
        hair = np.zeros(rgb.shape[:2], bool)
...
def shade_frame(frame: np.ndarray, smooth: bool = True,
                protect: bool = True) -> tuple[np.ndarray, int]:
    """Shade one 64x80 RGBA frame. Returns (new_frame, n_armor_pixels)."""
    armor, outline = classify(frame, protect=protect)
    if not armor.any():
        return frame, 0
    ...
    #    Disabled via --no-smooth for patterned legendary sheets, where
    #    diffusion would soften crisp authored geometry (zigzag, gadroon...).
    if smooth:
...
def shade_sheet(arr: np.ndarray, only_frame: int | None = None,
                smooth: bool = True, protect: bool = True):
...
def main():
    ...
    parser.add_argument("--no-protect", action="store_true",
                        help="do not skip skin/hair-coloured pixels; correct "
                             "for standalone armor sheets (see classify())")
    parser.add_argument("--no-smooth", action="store_true",
                        help="Skip the band-smoothing diffusion pre-pass "
                             "(use for patterned legendary sheets)")
