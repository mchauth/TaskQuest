#!/usr/bin/env python3
"""Preview the winged ankle boots ("talaria") for daily approval. For each class
and gender: full avatar (skin + base pants + base shirt + winged boots + hair)
across idle/walk/run/cheer/slash, plus one isolated boots-over-skin so the net-new
diagonal up-and-out feathered-wing geometry is clear. Writes to $OUTP
(default _PREVIEW_winged_boots.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_winged_boots_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_winged_boots.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "boots_warrior_legendary_wing", "Valkyr Warstriders"),
    ("mage", "boots_mage_legendary_wing", "Zephyr Voidwing Striders"),
    ("ranger", "boots_ranger_legendary_wing", "Skyrunner Treads"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE boots — net-new diagonal up-and-out winged-ankle geometry)",
        ...
        for gender in ("m", "f"):
            ...
            d.text((x + 2, yy + ch * Z), f"{gender} iso (wing+skin)", font=fsm, fill=(200, 200, 210, 255))
            yy += row_h
        y += class_h + pad
