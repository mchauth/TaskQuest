#!/usr/bin/env python3
"""Preview the raked poulaine war-sabaton boots for daily approval. For each class
and gender: full avatar (skin + base pants + base shirt + sabaton boots + hair)
across idle/walk/run/cheer/slash, plus one isolated boots-over-skin so the net-new
forward-raked toe geometry is clear. Writes to $OUTP (default
_PREVIEW_sabaton_boots.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_sabaton_boots_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_sabaton_boots.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "boots_warrior_legendary_sabaton", "Dreadclaw Sabatons"),
    ("mage", "boots_mage_legendary_sabaton", "Hexbite Striders"),
    ("ranger", "boots_ranger_legendary_sabaton", "Beastfang Treads"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE boots — net-new forward-raked poulaine-toe geometry)",
        ...
        for gender in ("m", "f"):
            ...
            d.text((x + 2, yy + ch * Z), f"{gender} iso (sabaton+skin)", font=fsm, fill=(200, 200, 210, 255))
            yy += row_h
        y += class_h + pad
