#!/usr/bin/env python3
"""Preview the paired outer SIDE-STRIPE legs (LEGS 8th axis) for daily approval.
For each class and gender: full avatar (skin + base shirt + these legs + boots +
hair) across idle/walk/run/cheer/slash, plus one isolated legs-over-skin so the
net-new paired-vertical stripe geometry is clear. Writes to $OUTP (default
_PREVIEW_sidestripe_legs.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_sidestripe_legs_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_sidestripe_legs.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "pants_warrior_legendary8", "Warlord's Field-Stripes"),
    ("mage", "pants_mage_legendary8", "Astral Piping"),
    ("ranger", "pants_ranger_legendary8", "Warden's Trail-Stripes"),
]
...
def avatar(stem, gender, fi, hair=True, dress=True):
    ...
    base.alpha_composite(frame(_open(f"{PREV}/{stem}{suf}.png"), fi))  # legs over boots
    if dress:
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, LEGENDARY legs — net-new paired outer SIDE-STRIPE geometry, 8th legs axis)",
        ...
        for gender in ("m", "f"):
            ...
            d.text((x + 2, yy + ch * Z), f"{gender} iso (legs+skin)", font=fsm, fill=(200, 200, 210, 255))
            yy += row_h
        y += class_h + pad
