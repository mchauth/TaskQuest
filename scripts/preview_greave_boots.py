#!/usr/bin/env python3
"""Preview the greave boots legendaries for daily approval. For each class and
gender: full avatar (skin + base pants + base shirt + greave boots + hair) across
idle/walk/run/cheer/slash, plus one isolated boots-over-skin so the net-new tall
shin-plate geometry is clear. Writes to $OUTP (default PREVIEW_greave_boots.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_greave_boots_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "PREVIEW_greave_boots.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "boots_warrior_legendary_greave", "Ironwrought Greaves"),
    ("mage", "boots_mage_legendary_greave", "Voidstep Greaves"),
    ("ranger", "boots_ranger_legendary_greave", "Thornstride Greaves"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE boots — net-new tall shin-plate greave geometry)",
        ...
        for gender in ("m", "f"):
            ...
            d.text((x + 2, yy + ch * Z), f"{gender} iso (greave+skin)", font=fsm, fill=(200, 200, 210, 255))
            yy += row_h
        y += class_h + pad
