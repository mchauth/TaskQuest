#!/usr/bin/env python3
"""Preview the cape-chest legendaries for daily approval. For each class and
gender: full avatar (skin+chest+hair) across idle/walk/run/cheer/slash, plus one
isolated chest-over-skin so the net-new draped-cape geometry is clear.
Writes to $OUTP (default PREVIEW_cape_legendary.png)."""
import os
...
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "PREVIEW_cape_legendary.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "shirt_warrior_legendary3", "Warlord's Warcape"),
    ("mage", "shirt_mage_legendary3", "Astral Shroud"),
    ("ranger", "shirt_ranger_legendary3", "Wildwood Cloak"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE chest — net-new draped-cape geometry)",
