#!/usr/bin/env python3
"""Approval preview for the 6th legendary sets (2026-07-24 batch):
  Rose Quartz Sovereign (warrior), Mithril Magus (mage), Crimson Warden (ranger).
...
right = the NEW legendary. Writes _PREVIEW_legendary6.png and a copy to the
...
OUTDIR = "/sessions/ecstatic-eager-cray/mnt/outputs"

# (title, source-tier token, preview-dir, legendary-suffix, swatch)
SETS = [
    ("Rose Quartz Sovereign (Warrior)", "rare1", "_warrior_legendary6_preview", "_rare6", (224, 134, 152)),
    ("Mithril Magus (Mage)", "mage4", "_mage_legendary6_preview", "_rare_mage6", (172, 190, 208)),
    ("Crimson Warden (Ranger)", "ranger4", "_ranger_legendary6_preview", "_rare_ranger6", (190, 34, 42)),
]
...
def main():
    ...
    title_h = 34
    label_h = 20
    # 4 avatars per row: M-src, M-new, F-src, F-new
    ...
    d.text((pad, 12), "TaskQuest — 6th Legendary Sets  (2026-07-24, awaiting approval)", font=fH, fill=(240, 240, 250))
    ...
    img.convert("RGB").save("_PREVIEW_legendary6.png")
    os.makedirs(OUTDIR, exist_ok=True)
    img.convert("RGB").save(os.path.join(OUTDIR, "PREVIEW_legendary6.png"))
    print("wrote _PREVIEW_legendary6.png and outputs/PREVIEW_legendary6.png", img.size)
