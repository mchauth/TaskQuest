#!/usr/bin/env python3
"""Preview the heel rowel-spur boots for daily approval. For each class and
gender: full avatar (skin + base pants + base shirt + spur boots + hair) across
idle/walk/run/cheer/slash, plus one isolated boots-over-skin so the net-new
heel-spur + rowel-wheel geometry is clear. Writes to $OUTP (default
_PREVIEW_spur_boots.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_spur_boots_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_spur_boots.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "boots_warrior_legendary_spur", "Ironclad Rowel-Spurs"),
    ("mage", "boots_mage_legendary_spur", "Astral Rowel-Striders"),
    ("ranger", "boots_ranger_legendary_spur", "Wildspur Treads"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE boots — net-new heel-spur + rowel-wheel geometry)",
        ...
        for gender in ("m", "f"):
            ...
            d.text((x + 2, yy + ch * Z), f"{gender} iso (spur+skin)", font=fsm, fill=(200, 200, 210, 255))
            yy += row_h
        y += class_h + pad
