#!/usr/bin/env python3
"""Preview the heraldic-chevron chest legendaries (9th chest axis) for daily
approval. For each class and gender: full avatar (skin+chest+hair) across
idle/walk/run/cheer/slash, plus one isolated chest-over-skin (no hair) so the
net-new symmetric down-pointing V geometry is clear. Writes to $OUTP (default
_PREVIEW_chevron_legendary.png)."""
import os
from PIL import Image, ImageDraw, ImageFont

CH = "sprites/preview_assets/char"
PREV = "_chevron_legendary_preview"
FW, FH, COLS = 80, 64, 10
OUT = os.environ.get("OUTP", "_PREVIEW_chevron_legendary.png")
FRAMES = [(0, "idle"), (12, "walk"), (22, "run"), (41, "cheer"), (52, "slash")]
CLASSES = [
    ("warrior", "shirt_warrior_legendary9", "Sovereign's Chevron"),
    ("mage", "shirt_mage_legendary9", "Astral Chevron"),
    ("ranger", "shirt_ranger_legendary9", "Warden's Chevron"),
]
...
def main():
    ...
    for cls, stem, disp in CLASSES:
        d.text((pad, y), f"{disp}  ({cls}, HYPER-RARE chest — net-new HERALDIC CHEVRON (down-V), 9th chest axis (repaint, QA-safe))",
