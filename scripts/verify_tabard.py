#!/usr/bin/env python3
"""Rigorous per-sheet verification for the tabard legendaries.
For each of the 6 sheets, vs its source t4-chest sheet:
  1. source px dropped  = source silhouette px NOT present in output (should be 0)
  ...
  3. accent-caused multi-component frames = frames whose OUTPUT has >1 connected
     component beyond what the SOURCE already has (should be 0 -> every tabard fused)
  4. accent strays = output opaque px that are neither in-source nor 4-connected
...
PAIRS = [
    ("armor_chest_4", "shirt_warrior_legendary4"),
    ("shirt_mage4", "shirt_mage_legendary4"),
    ("shirt_ranger4", "shirt_ranger_legendary4"),
]
