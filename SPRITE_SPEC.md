...
>
> Companion tooling: `scripts/sprite_finish.py` (**the finishing pass — see §0**),
> `scripts/sprite_shade.py` (shader), `scripts/sprite_qa.py` (QA),
...
## 0. The finishing pass — EVERY generator must call this

`scripts/sprite_finish.py` is the single entry point for everything approved on
2026-08-01: no-smooth shading, sculptural chest plates, helmet visors, class-hat folds.
**Do not re-implement shading in a new `gen_*.py`.** Replace the per-script `shade(...)`
call with:
...
The filename is passed because the **variant** is chosen from a hash of it, so a sheet
always regenerates identically and male/female of the same item always match.

| Slot | Applied |
|---|---|
| all | `sprite_shade.shade_sheet(smooth=False)` — **always `--no-smooth`**; the diffusion pre-pass blurs authored pattern geometry |
| shirt | pauldrons, metallic sheen, gorget, sternum + band separation (`SEPARATION_VARIANTS`: full / band_only) |
| helmet | visor + dark outline ring (`VISOR_VARIANTS`: slit / breath / nasal / grille / great) |
| helmet, if visor coverage < `HELM_MIN_PX` | it's open headgear → class-hat brim/crease folds instead |
| pants / boots | shading only |

Backfilling existing sheets:

```bash
python3 scripts/sprite_finish.py _some_preview/          # a directory, or files
python3 scripts/sprite_finish.py --force <paths>         # re-finish already-stamped
```

Finished files carry a PNG text chunk `TaskQuestFinish=<VERSION>`; `finish_file` skips
stamped sheets, so a bulk run is **safe to repeat and cannot double-apply shading**.
Bump `VERSION` in `sprite_finish.py` when the chain changes to force a re-finish.
